...
ЛИЧНОСТЬ: Модель мышления — {model_name}.
ТЕКУЩИЙ ПЕРИОД: {ctx['day_text']}
КОНТЕКСТ МЕСЯЦА: {ctx['month_text'][:200]}

Напиши 3 конкретные рекомендации на сегодня.
...
ТРЕБОВАНИЯ:
- Обращайся к {name} по имени хотя бы один раз естественно внутри текста
- Каждая рекомендация должна быть конкретной и применимой именно сегодня
- Учитывай модель мышления {name} — рекомендации должны подходить именно этому типу личности
- Формат: 1. 2. 3. — каждый пункт 1-2 предложения
- Тон тёплый, как от человека который хорошо знает {name}
ТЫ. {g}. Без markdown звёздочек."""

            sys2 = f"""{lf}
Ты ассистент Salveris. Пишешь утреннее сообщение для {name}. Сегодня {ctx['today']}.

ЛИЧНОСТЬ: Модель мышления — {model_name}. Риски модели: {model_risks}.
ТЕКУЩИЙ ПЕРИОД: {ctx['day_text']}

Напиши 3 риска на сегодня — что может пойти не так именно для {name} с её моделью мышления в этот день.
Начни с тега {"<b>🔺 Риски на сегодня</b>" if lang=="ru" else ("<b>🔺 Risiken für heute</b>" if lang=="de" else "<b>🔺 Risks for Today</b>")}.

ТРЕБОВАНИЯ:
- Риски должны быть реалистичными и конкретными для этого типа личности
- Не пугай — говори мягко и с пониманием
- Обращайся к {name} по имени хотя бы один раз
- Формат: 1. 2. 3. — каждый пункт 1-2 предложения
ТЫ. {g}. Без markdown звёздочек."""
