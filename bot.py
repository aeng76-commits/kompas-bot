...
def get_profile_prompt(lang, user, section):
    ...
    if section == "me":
        instruction = f"""Дай развёрнутый анализ личности {name} на основе модели мышления.
Структура:
✨ Основа личности — кто ты есть
🌿 Сильные стороны — конкретно и образно
🔺 Слабые стороны — мягко, без диагнозов
🌱 Как превратить слабости в силу — практично
...
Формула роста: {mi.get("formula","")}"""
        ...
        instruction = f"""Дай развёрнутый анализ личного года для {name}.
Структура:
🧭 Что означает этот период
✨ Главные возможности года
🔺 Главные риски года
🌿 Рекомендации на этот год
🌱 На что обратить внимание прямо сейчас
Используй разделитель ・・・・・・・・・・ между блоками.
ЛИЧНЫЙ ГОД: {year_text}"""

    elif section == "month":
        instruction = f"""Дай развёрнутый анализ личного месяца для {name}.
Структура:
🧭 Что означает этот период
✨ Главные возможности месяца
🔺 Главные риски месяца
🌿 Рекомендации на этот месяц
🌱 Конкретные шаги на ближайшие 7-14 дней
Используй разделитель ・・・・・・・・・・ между блоками.
...
Говори только по-{'русски' if lang == 'ru' else ('немецки' if lang == 'de' else 'английски')}.
Обращайся на ТЫ. Имя используй точно: {name}.
Живой тёплый стиль. Только чистый текст без markdown.
Нельзя: нумерология, вибрация, энергия, трансформация, вызовы.
